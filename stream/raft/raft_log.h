...
class RaftLog {
 ...
 public:
  static RaftLogPtr MakeRaftLog(seastar::shared_ptr<Storage> storage,
                                ...
                                spdlog::logger *logger, uint64_t group = 0,
  ...
  seastar::future<Result<SnapshotPtr>> Snapshot();
  ...
  seastar::future<Result<uint64_t>> Term(uint64_t i);
  seastar::future<uint64_t> LastTerm();

  seastar::future<Result<std::vector<EntryPtr>>> Entries(uint64_t i,
                                                         uint64_t max_size);
  ...
  seastar::future<Result<std::vector<EntryPtr>>> Slice(uint64_t lo, uint64_t hi,
                                                       uint64_t max_size);
