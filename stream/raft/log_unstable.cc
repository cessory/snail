...
void Unstable::TruncateAndAppend(const std::vector<EntryPtr>& ents) {
    ...
    if (after == offset_ + entries_.size()) {
        ...
        LOG_INFO("[{}-{}] replace the unstable entries from index {}", group_,
                 id_, after);
        offset_ = after;
        entries_.clear();
    } else {
        LOG_INFO("[{}-{}] truncate the unstable entries before index {}",
                 group_, id_, after);
...
std::vector<EntryPtr> Unstable::Slice(uint64_t lo, uint64_t hi) {
    ...
    if (lo > hi) {
        LOG_FATAL_THROW("[{}-{}] invalid unstable.slice {} > {}", group_, id_,
                        lo, hi);
    }

    auto upper = offset_ + entries_.size();
    if (lo < offset_ || hi > upper) {
        LOG_FATAL_THROW("[{}-{}] unstable.slice[{},{}) out of bound [{},{}]",
                        group_, id_, lo, hi, offset_, upper);
    }
