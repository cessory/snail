...
SEASTAR_THREAD_TEST_CASE(ProgressLeaderTest) {
  ...
  for (int i = 0; i < 5; i++) {
    ...
    ASSERT_EQ(pr->state(), snail::raft::StateType::StateReplicate);
    ASSERT_EQ(pr->match(), i + 1);
    ASSERT_EQ(pr->next(), pr->match() + 1);

    auto err = raft->Step(m).get0();
    ASSERT_EQ(err, RAFT_OK);
  }
}
...
SEASTAR_THREAD_TEST_CASE(ProgressResumeByHeartbeatRespTest) {
  ...
  raft->Step(m).get();
  ASSERT_TRUE(raft->prs_->progress()[2]->probe_sent());
  ...
  raft->Step(m).get();
  ASSERT_FALSE(raft->prs_->progress()[2]->probe_sent());
}
...
SEASTAR_THREAD_TEST_CASE(ProgressFlowControlTest) {
  ...
  auto raft =
      seastar::make_lw_shared<snail::raft::Raft>(spdlog::default_logger_raw());
  raft->Init(cfg).get();
  raft->BecomeCandidate();
  raft->BecomeLeader().get();
  ...
  ASSERT_EQ(ms.size(), 1);
  ASSERT_EQ(ms[0]->type, snail::raft::MessageType::MsgApp);
  ...
  for (int i = 0; i < ms.size(); i++) {
    ASSERT_EQ(ms[i]->type, snail::raft::MessageType::MsgApp);
    ASSERT_EQ(ms[i]->entries.size(), 2);
  }
...
SEASTAR_THREAD_TEST_CASE(UncommittedEntryLimitTest) {
  ...
  raft->BecomeCandidate();
  raft->BecomeLeader().get();
  ASSERT_EQ(raft->uncommitted_size_, 0);
  ...
  m->entries.push_back(ent);
  auto err = raft->Step(m).get0();
  ASSERT_EQ(err, RAFT_ERR_PROPOSALDROPPED);

  auto ms = std::move(raft->msgs_);
  ASSERT_EQ(ms.size(), maxEntries * numFollowers);
  ...
  ms = std::move(raft->msgs_);
  ASSERT_EQ(ms.size(), 2 * numFollowers);
  raft->ReduceUncommittedSize(propEnts);
  ASSERT_EQ(raft->uncommitted_size_, 0);
}
...
static void testLeaderElection(bool preVote) {
  ...
  testItem tests[6] = {
      ...
      {newNetworkWithConfig(cfg, {nullptr, nullptr, nopStepper}),
       snail::raft::RaftState::StateLeader, 1},
      {newNetworkWithConfig(cfg, {nullptr, nopStepper, nopStepper}), candState,
       candTerm},
  ...
  for (int i = 0; i < 6; i++) {
    ...
    auto sm = seastar::dynamic_pointer_cast<RaftSM>(tests[i].net->peers_[1]);
    ASSERT_EQ(sm->raft_->state_, tests[i].state);
    ASSERT_EQ(sm->raft_->term_, tests[i].expTerm);
  }
}
...
SEASTAR_THREAD_TEST_CASE(LearnerElectionTimeoutTest) {
  ...
  ASSERT_EQ(n2->state_, snail::raft::RaftState::StateFollower);
}
...
SEASTAR_THREAD_TEST_CASE(LearnerPromotionTest) {
  ...
  ASSERT_FALSE(n1->state_ == snail::raft::RaftState::StateLeader);
  n1->randomized_election_timeout_ = n1->election_timeout_;
  for (int i = 0; i < n1->election_timeout_; i++) {
    n1->tick_().get();
  }
  ASSERT_TRUE(n1->state_ == snail::raft::RaftState::StateLeader);
  ASSERT_EQ(n2->state_, snail::raft::RaftState::StateFollower);
  ...
  m->type = snail::raft::MessageType::MsgBeat;
  nt->Send({m}).get();
  ASSERT_EQ(n1->state_, snail::raft::RaftState::StateFollower);
  ASSERT_EQ(n2->state_, snail::raft::RaftState::StateLeader);
}
...
SEASTAR_THREAD_TEST_CASE(LearnerCanVoteTest) {
  ...
  ASSERT_EQ(n2->msgs_.size(), 1);
  ASSERT_FALSE((n2->msgs_[0]->type != snail::raft::MessageType::MsgVoteResp &&
                !n2->msgs_[0]->reject));
}
...
static void testLeaderCycle(bool preVote) {
  ...
  for (uint64_t campaignerID = 1; campaignerID <= 3; campaignerID++) {
    ...
    for (auto iter : n->peers_) {
      ...
      ASSERT_FALSE(sm->raft_->id_ == campaignerID &&
...
static void testLeaderElectionOverwriteNewerLogs(bool preVote) {
  ...
  ASSERT_EQ(sm1->raft_->state_, snail::raft::RaftState::StateFollower);
  ASSERT_EQ(sm1->raft_->term_, 2);
  ...
  m->type = snail::raft::MessageType::MsgHup;
  n->Send({m}).get();
  ASSERT_EQ(sm1->raft_->state_, snail::raft::RaftState::StateLeader);
  ASSERT_EQ(sm1->raft_->term_, 3);
...
static void testVoteFromAnyState(snail::raft::MessageType vt) {
  for (int st = (int)snail::raft::RaftState::StateFollower;
    ...
    auto err = raft->Step(m).get0();
    ASSERT_EQ(err, RAFT_OK);
    ASSERT_EQ(raft->msgs_.size(), 1);
    ASSERT_EQ(raft->msgs_[0]->type,
              (vt == snail::raft::MessageType::MsgVote
                   ? snail::raft::MessageType::MsgVoteResp
                   : snail::raft::MessageType::MsgPreVoteResp));
    ASSERT_FALSE(raft->msgs_[0]->reject);
    if (vt == snail::raft::MessageType::MsgVote) {
      ASSERT_EQ(raft->state_, snail::raft::RaftState::StateFollower);
      ASSERT_EQ(raft->term_, newTerm);
      ASSERT_EQ(raft->vote_, 2);
    } else {
      ASSERT_EQ(raft->state_, (snail::raft::RaftState)st);
      ASSERT_EQ(raft->term_, origTerm);
      ASSERT_FALSE(raft->vote_ != 0 && raft->vote_ != 1);
    }
  }
}
...
SEASTAR_THREAD_TEST_CASE(LogReplicationTest) {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    for (auto iter : tests[i].net->peers_) {
      auto id = iter.first;
      auto sm = seastar::dynamic_pointer_cast<RaftSM>(iter.second);
      ASSERT_EQ(sm->raft_->raft_log_->committed(), tests[i].wcommitted);
      ...
      ASSERT_EQ(ents.size(), props.size());

      for (int j = 0; j < props.size(); j++) {
        ASSERT_EQ(ents[j]->data(), props[j]->entries[0]->data());
      }
    }
  }
}
...
SEASTAR_THREAD_TEST_CASE(LearnerLogReplicationTest) {
  ...
  ASSERT_EQ(n1->state_, snail::raft::RaftState::StateLeader);
  ...
  ASSERT_EQ(n1->raft_log_->committed(), nextCommitted);
  ASSERT_EQ(n1->raft_log_->committed(), n2->raft_log_->committed());
  ASSERT_EQ(n2->raft_log_->committed(), n1->prs_->progress()[2]->match());
}
...
SEASTAR_THREAD_TEST_CASE(SingleNodeCommitTest) {
  ...
  ASSERT_EQ(sm->raft_->raft_log_->committed(), 3);
}
...
SEASTAR_THREAD_TEST_CASE(CannotCommitWithoutNewTermEntryTest) {
  ...
  auto sm = seastar::dynamic_pointer_cast<RaftSM>(tt->peers_[1]);
  ASSERT_EQ(sm->raft_->raft_log_->committed(), 1);
  tt->Recover();
  ...
  m->entries.push_back(ent);
  tt->Send({m}).get();
  ASSERT_EQ(sm->raft_->raft_log_->committed(), 5);
}
...
SEASTAR_THREAD_TEST_CASE(CommitWithoutNewTermEntryTest) {
  ...
  m->type = snail::raft::MessageType::MsgHup;
  tt->Send({m}).get();
  ASSERT_EQ(sm->raft_->raft_log_->committed(), 4);
}
...
SEASTAR_THREAD_TEST_CASE(DuelingCandidatesTest) {
  ...
  ASSERT_EQ(sm->raft_->state_, snail::raft::RaftState::StateLeader);

  sm = seastar::dynamic_pointer_cast<RaftSM>(nt->peers_[3]);
  ASSERT_EQ(sm->raft_->state_, snail::raft::RaftState::StateCandidate);
  ...
  auto wlog =
      snail::raft::RaftLog::MakeRaftLog(store, 0, spdlog::default_logger_raw());
  ...
  testItem tests[3] = {
      ...
      {c, snail::raft::RaftState::StateFollower, 2,
       snail::raft::RaftLog::MakeRaftLog(newTestMemoryStorage(),
                                         std::numeric_limits<uint64_t>::max(),
                                         spdlog::default_logger_raw())}};

  for (int i = 0; i < 3; i++) {
    ASSERT_EQ(tests[i].sm->state_, tests[i].state);
    ASSERT_EQ(tests[i].sm->term_, tests[i].term);
    auto p = nt->peers_[i + 1];
    if (p && p->Type() == StateMachineType::RaftStateMachine) {
      sm = seastar::dynamic_pointer_cast<RaftSM>(p);
      ASSERT_EQ(sm->raft_->raft_log_->committed(),
                tests[i].raftLog->committed());
      ASSERT_EQ(sm->raft_->raft_log_->applied(), tests[i].raftLog->applied());
...
SEASTAR_THREAD_TEST_CASE(DuelingPreCandidatesTest) {
  ...
  sm = seastar::dynamic_pointer_cast<RaftSM>(nt->peers_[3]);
  ASSERT_EQ(sm->raft_->state_, snail::raft::RaftState::StateFollower);
...
SEASTAR_THREAD_TEST_CASE(CandidateConcedeTest) {
  ...
  ASSERT_EQ(a->raft_->state_, snail::raft::RaftState::StateFollower);
  ASSERT_EQ(a->raft_->term_, 1);
  ...
  for (auto iter : tt->peers_) {
    ...
    if (p && p->Type() == StateMachineType::RaftStateMachine) {
      ...
      ASSERT_EQ(sm->raft_->raft_log_->committed(), wlog->committed());
      ASSERT_EQ(sm->raft_->raft_log_->applied(), wlog->applied());
      auto ents1 = sm->raft_->raft_log_->AllEntries().get0();
...
SEASTAR_THREAD_TEST_CASE(ProposalTest) {
  ...
  for (int i = 0; i < 5; i++) {
    auto tt = tests[i].tt;
    auto send = [tt, success = tests[i].success](snail::raft::MessagePtr m) {
      try {
        tt->Send({m}).get();
      } catch (...) {
...
SEASTAR_THREAD_TEST_CASE(CommitTest) {
  ...
  for (int i = 0; i < 14; i++) {
    ...
    ASSERT_EQ(raft->raft_log_->committed(), tests[i].w);
  }
}
...
SEASTAR_THREAD_TEST_CASE(PastElectionTimeoutTest) {
  ...
  for (int i = 0; i < 6; i++) {
    ...
    ASSERT_FLOAT_EQ(got, tests[i].wprobability);
  }
}

SEASTAR_THREAD_TEST_CASE(StepIgnoreOldTermMsgTest) {
  bool called = false;
  auto fakeStep = [&called](snail::raft::MessagePtr m) -> seastar::future<int> {
    called = true;
    return seastar::make_ready_future<int>(RAFT_OK);
  };
...
SEASTAR_THREAD_TEST_CASE(HandleMsgAppTest) {
  ...
  for (int i = 0; i < 11; i++) {
    ...
    ASSERT_EQ(raft->raft_log_->LastIndex(), tests[i].windex);
    ASSERT_EQ(raft->raft_log_->committed(), tests[i].wcommit);
    auto msgs = std::move(raft->msgs_);
    ASSERT_EQ(msgs.size(), 1);
    ASSERT_EQ(msgs[0]->reject, tests[i].wreject);
  }
}
...
SEASTAR_THREAD_TEST_CASE(HandleHeartbeatTest) {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    auto msgs = std::move(raft->msgs_);
    ASSERT_EQ(msgs.size(), 1);
    ASSERT_EQ(msgs[0]->type, snail::raft::MessageType::MsgHeartbeatResp);
  }
}
...
SEASTAR_THREAD_TEST_CASE(HandleHeartbeatRespTest) {
  ...
  auto msgs = std::move(raft->msgs_);
  ASSERT_EQ(msgs.size(), 1);
  ASSERT_EQ(msgs[0]->type, snail::raft::MessageType::MsgApp);
...
SEASTAR_THREAD_TEST_CASE(RaftFreesReadOnlyMemTest) {
  ...
  auto msgs = std::move(raft->msgs_);
  ASSERT_EQ(msgs.size(), 1);
  ASSERT_EQ(msgs[0]->type, snail::raft::MessageType::MsgHeartbeat);
  ASSERT_EQ(msgs[0]->context, ctx);
  ASSERT_EQ(raft->read_only_->read_index_queue_.size(), 1);
  ASSERT_EQ(raft->read_only_->pending_read_index_.size(), 1);
  auto iter = raft->read_only_->pending_read_index_.find(
      seastar::sstring(ctx.get(), ctx.size()));
  ASSERT_TRUE(iter != raft->read_only_->pending_read_index_.end());
  ...
  ASSERT_EQ(raft->read_only_->read_index_queue_.size(), 0);
  ASSERT_EQ(raft->read_only_->pending_read_index_.size(), 0);
...
SEASTAR_THREAD_TEST_CASE(MsgAppRespWaitResetTest) {
  ...
  m->index = 1;
  raft->Step(m).get();
  ASSERT_EQ(raft->raft_log_->committed(), 1);
  msgs = std::move(raft->msgs_);
  ...
  msgs = std::move(raft->msgs_);
  ASSERT_EQ(msgs.size(), 1);
  ASSERT_FALSE(msgs[0]->type != snail::raft::MessageType::MsgApp ||
               msgs[0]->to != 2);
  ...
  ASSERT_FALSE(msgs[0]->type != snail::raft::MessageType::MsgApp ||
               msgs[0]->to != 3);
  ASSERT_FALSE(msgs[0]->entries.size() != 1 ||
               msgs[0]->entries[0]->index() != 2);
}
...
static void testRecvMsgVote(snail::raft::MessageType msgType) {
  ...
  for (int i = 0; i < 21; i++) {
    ...
    switch (tests[i].state) {
      case snail::raft::RaftState::StateFollower:
        raft->step_ =
            [raft](snail::raft::MessagePtr m) -> seastar::future<int> {
    ...
    raft->raft_log_ = snail::raft::RaftLog::MakeRaftLog(
        store, 0, raft->logger_, raft->group_, raft->id_);
    raft->raft_log_->unstable_.set_offset(3);
    auto term = std::max(raft->raft_log_->LastTerm().get0(), tests[i].logTerm);
    raft->term_ = term;
    ...
    auto msgs = std::move(raft->msgs_);
    ASSERT_EQ(msgs.size(), 1);
    ASSERT_EQ(msgs[0]->type, msgType == snail::raft::MessageType::MsgVote
                                 ? snail::raft::MessageType::MsgVoteResp
                                 : snail::raft::MessageType::MsgPreVoteResp);
    ASSERT_EQ(msgs[0]->reject, tests[i].wreject);
  }
...
SEASTAR_THREAD_TEST_CASE(StateTransitionTest) {
  ...
  testItem tests[16] = {
      ...
      {snail::raft::RaftState::StateLeader, snail::raft::RaftState::StateLeader,
       true, 0, 1},
  };
  ...
  for (int i = 0; i < 16; i++) {
    ...
    try {
      ...
      ASSERT_EQ(raft->term_, tests[i].wterm);
      ASSERT_EQ(raft->lead_, tests[i].wlead);
    } catch (...) {
      ASSERT_FALSE(tests[i].wallow);
    }
  }
}
...
SEASTAR_THREAD_TEST_CASE(AllServerStepdownTest) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    for (int j = 0; j < 2; j++) {
      ...
      m->log_term = tterm;
      raft->Step(m).get();
      ASSERT_EQ(raft->state_, tests[i].wstate);
      ASSERT_EQ(raft->term_, tests[i].wterm);
      ASSERT_EQ(raft->raft_log_->LastIndex(), tests[i].windex);
      auto ents = raft->raft_log_->AllEntries().get0();
      ASSERT_EQ(ents.size(), tests[i].windex);
      uint64_t wlead = 2;
      if (tmsgTypes[j] == snail::raft::MessageType::MsgVote) {
        wlead = 0;
      }
      ASSERT_EQ(raft->lead_, wlead);
    }
  }
}
...
static void testCandidateResetTerm(snail::raft::MessageType mt) {
  ...
  ASSERT_EQ(a->state_, snail::raft::RaftState::StateLeader);
  ASSERT_EQ(b->state_, snail::raft::RaftState::StateFollower);
  ASSERT_EQ(c->state_, snail::raft::RaftState::StateFollower);
  ...
  ASSERT_EQ(c->state_, snail::raft::RaftState::StateCandidate);
...
SEASTAR_THREAD_TEST_CASE(LeaderStepdownWhenQuorumActiveTest) {
  ...
  ASSERT_EQ(raft->state_, snail::raft::RaftState::StateLeader);
}
...
SEASTAR_THREAD_TEST_CASE(LeaderSupersedingWithCheckQuorumTest) {
  ...
  ASSERT_EQ(c->state_, snail::raft::RaftState::StateLeader);
}
...
SEASTAR_THREAD_TEST_CASE(LeaderElectionWithCheckQuorumTest) {
  ...
  ASSERT_EQ(a->state_, snail::raft::RaftState::StateFollower);
  ASSERT_EQ(c->state_, snail::raft::RaftState::StateLeader);
}
...
SEASTAR_THREAD_TEST_CASE(DisruptiveFollowerTest) {
  ...
  ASSERT_EQ(n1->state_, snail::raft::RaftState::StateLeader);
  ASSERT_EQ(n2->state_, snail::raft::RaftState::StateFollower);
  ASSERT_EQ(n3->state_, snail::raft::RaftState::StateFollower);
  ...
  ASSERT_EQ(n1->state_, snail::raft::RaftState::StateLeader);
  ASSERT_EQ(n2->state_, snail::raft::RaftState::StateFollower);
  ASSERT_EQ(n3->state_, snail::raft::RaftState::StateCandidate);
...
SEASTAR_THREAD_TEST_CASE(DisruptiveFollowerPreVoteTest) {
  ...
  ASSERT_EQ(n1->state_, snail::raft::RaftState::StateLeader);
  ASSERT_EQ(n2->state_, snail::raft::RaftState::StateFollower);
  ASSERT_EQ(n3->state_, snail::raft::RaftState::StatePreCandidate);
  ASSERT_EQ(n1->term_, 2);
  ASSERT_EQ(n2->term_, 2);
...
SEASTAR_THREAD_TEST_CASE(ReadOnlyOptionSafeTest) {
  ...
  for (int i = 0; i < 6; i++) {
    ...
    ASSERT_TRUE(tests[i].sm->read_states_.size() > 0);
    ASSERT_EQ(tests[i].sm->read_states_[0].index, tests[i].wri);
    ASSERT_EQ(tests[i].sm->read_states_[0].request_ctx, tests[i].wctx);
