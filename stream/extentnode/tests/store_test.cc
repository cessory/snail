...
SEASTAR_THREAD_TEST_CASE(write_test) {
    auto ok = snail::stream::Store::Format("/dev/sdb", 1,
                                           snail::stream::DevType::HDD, 1)
                  .get0();
    BOOST_REQUIRE(ok);
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (free < 200) {
            ...
            uint64_t sector_free = kSectorSize - len % kSectorSize;
            if (sector_free < 200) {
                char *p = b.get_write();
                uint32_t crc = crc32_gzip_refl(
        ...
        std::cout << "chunk scrc=" << extent_ptr->chunks.back().scrc
                  << " i=" << i << std::endl;
        if (!s) {
            std::cout << "write offset=" << offset << " i=" << i
                      << " extent len=" << extent_ptr->len << std::endl;
        }
        BOOST_REQUIRE(s.OK());
        offset += 200;
        buffers.clear();
    }
    BOOST_REQUIRE_EQUAL(extent_ptr->len, 100 * 1024 * 1024);
}
