...
static constexpr int kHeaderSize = 4 + 8;                // crc + ver
static constexpr int kRecordHeaderSize = 4 + 8 + 2 + 1;  // len + type
static constexpr int kMaxMemSize = 32768;

// log format
// |   header    |             data                         |
// | header(512B) |    record(512B)  |   ....  |   record   |
//
// record
...
void Log::ResetMem() {
    ...
    if (offset_ == super_.pt[current_log_pt_].start) {
        BigEndian::PutUint64(p + 4, version_);
        BigEndian::PutUint32(p, crc32_gzip_refl(0, p + 4, sizeof(version_)));
        mem_size_ = super_.sector_size;
    }
}

seastar::future<> Log::BackgroundFlush() {
    ...
    for (auto iter : immu_chunk_mem_) {
        ...
        if (buf_vec.size() == 0) {
            ...
            if (iter->second.index == prev_index + 1) {
                ...
                status_ = co_await dev_ptr_->Write(off, std::move(iov));
                if (!status_.OK()) {
    ...
    if (!iov.empty()) {
        uint64_t off = super_.pt[1].start + first_index * sector_size;
        status_ = co_await dev_ptr_->Write(off, std::move(iov));
        if (!status_.OK()) {
            background_sem_.broken();
            co_return;
...
seastar::future<Status<>> Log::Flush() {
    char *p = mem_.get_write();
    Status<> s;
    ...
    s = co_await dev_ptr_->Write(offset_, mem_.get(), mem_size_);
    co_return s;
}

void Log::AppendToMem(const std::variant<ExtentEntry, ChunkEntry> &entry) {
    char *p = mem_.get_write() + mem_size_;
    uint32_t sector_size = super_.sector_size;
    ...
    if (entry.index() == 0) {
        ...
        data_len = static_cast<uint16_t>(kExtentEntrySize);
        type = EntryType::Extent;
    } else {
        const ChunkEntry &ent = std::get<1>(entry);
        ent.MarshalTo(p + kRecordHeaderSize);
        data_len = static_cast<uint16_t>(kChunkEntrySize);
        type = EntryType::Chunk;
    }
    BigEndian::PutUint64(p + 4, version_);
    BigEndian::PutUint16(p + 12, data_len);
    *(p + 14) = static_cast<char>(type);
    BigEndian::PutUint32(
        p, crc32_gzip_refl(0, p + 4, kRecordHeaderSize + data_len - 4));
    p += sector_size;
    mem_size_ += sector_size;
}
...
seastar::future<> Log::LoopRun() {
    ...
    while (!stop_) {
        ...
        while (!queue_.empty()) {
            ...
            size_t n = mem_size_ + sector_size;
            if (n > mem_.size() || n > free_size) {
                auto s = co_await Flush();  // dump mem to disk
                if (s.OK()) {
                    offset_ += mem_size_;
                }
                ResetMem();
