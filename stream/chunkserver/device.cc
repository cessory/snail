 ...
 public:
  ...
  seastar::future<Status<>> Write(uint64_t pos, const char* b,
    ...
    try {
      ...
      if (n != len) {
        co_return Status<>(ErrCode::ErrWriteDisk, "write bytes not expect");
      }
    } catch (std::system_error& e) {
      if (e.code().value() == EIO || e.code().value() == EBADF) {
        co_return Status<>(ErrCode::ErrDisk, e.code().message());
      }
      co_return Status<>(ErrCode::ErrSystem, e.code().message());
    } catch (std::exception& e) {
      co_return Status<>(ErrCode::ErrWriteDisk, e.what());
    }
  ...
  seastar::future<std::pair<int, size_t>> Read(uint64_t pos, char* b,
    ...
    try {
      ...
      if (e.code().value() == EIO || e.code().value() == EBADF) {
        st.Set(ErrCode::ErrDisk, e.code().message());
      } else {
        st.Set(ErrCode::ErrSystem, e.code().message());
      }
    } catch (std::exception& e) {
      st.Set(ErrCode::ErrSystem, e.what());
    }
    co_return st;
  }
...
class SpdkDevice : public Device, public enable_shared_from_this<SpdkDevice> {
      ...
      if nsid
        == req->dev_ptr->ns_id_ {
          ...
          if (!spdk_nvme_ns_is_active(ns)) {
            SPDLOG_ERROR("device {} ns: {} is not active", trid->traddr, nsid)
            req->pr.set_value(-1);
            return
          }
          ...
          req->dev_ptr->sector_size_ = spdk_nvme_ns_get_sector_size(ns);
          req->pr.set_value(0);
          return;
        }
    }
 ...
 public:
  static seastar::future<DevicePtr> Open(const std::string_view name,
    ...
    seastar::shared_ptr<SpdkDevice> ptr = seastar::make_shared<SpdkDevice>();
    spdk_nvme_trid_populate_transport(&ptr->trid_, SPDK_NVME_TRANSPORT_PCIE);
    ...
    req->dev_ptr = ptr;
    ptr->probe_ctx_ = spdk_nvme_probe_async(
        &ptr->trid_, req, SpdkDevice::probe_cb, SpdkDevice::attach_cb, NULL);

    ptr->poller_ = seastar::reactor::poller::simple([ptr]() { ptr->poll(); });
  ...
  seastar::future<Status<>> Write(uint64_t pos, std::vector<iovec> iov) {
    ...
    auto res = spdk_nvme_ns_cmd_writev(
        ns_, qpairs_[qpairs_idx], (pos / sector_size_), (len / sector_size_),
